...
A slow drum beat and a sustained, high-pitched whistle are physically doing the exact same thing: creating repeating patterns of amplitude energy over time. The underlying mechanics are identical, and our brain is doing the same predictive work. Our perception simply shifts based on the rate of the pattern. Rhythm and pitch are actually the very same phenomenon, just experienced at different speeds. A composition smoothly accelerating from 4 beats per second to 40 per second would cross this boundary smoothly.
