...
1. **Every anchor or floating period is the referenced neighbor of at most one Floating period, per side.** This prevents two Floating periods from competing for the same neighbor on the same side, and prevents a single Floating period from branching toward two different neighbors on the same side. Together these guarantee the whole adjacency structure decomposes into disjoint linear chains.
